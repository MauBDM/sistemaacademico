...
<body>
    <nav class="navbar navbar-expand-sm bg-dark navbar-blue">
        ...
        <div class="navbar-header"><img src="../img/logo.png" alt="" width="40px">
          <a class="navbar-brand" href="#"><i class="fa fa-graduation-cap"></i> <?php echo "Calificaciones ".$nombreGrado." Grado"; ?></a>
        </div>
    ...
    <div class="container"><div id="chat">
        <form id="frmmatricula" method="POST" action="editarNotas.php" autocomplete="off">
        <div class="form-row">
          <input type="hidden" name="grado" id="grado" value="<?php echo $idgrado; ?>">
          <div class="col-6">
            <select class="selectpicker form-control" data-show-subtext="true" data-live-search="true" name="asignatura" id="asignatura" required="" onchange="activar()">
              <option disabled="" selected="">Seleccionar asignatura</option>
                <?php
...
<script type="text/javascript">
        ...
        $.ajax({
          ...
          success:function(r){
            if(r==1){
              ...
              alert("Estudiante ya estaba registrado!\nNo se permite duplicar registros.");
            }
          }
        });
        return false;
  ...
  function Cargar(){
    ...
    $('#datagrid').load('verNotas.php?idgrado='+grado+'&idasignatura='+asignatura);
    /*$("#loader").fadeIn('slow');
      $.ajax({
