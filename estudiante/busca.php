...
//contamos los valores encontrados y realiza consulta.
      $count_query = "SELECT estudiante.*, responsable.nombres as responsableNombre FROM $Table inner join responsable on estudiante.duiResponsable = responsable.dui $Where and estudiante.idestado='1'";
...
// Consulta de la paginación
      $sql = "SELECT estudiante.*, responsable.nombres as responsableNombre FROM $Table inner join responsable on estudiante.duiResponsable = responsable.dui $Where and estudiante.idestado='1' ORDER BY estudiante.nie desc LIMIT $offset,$per_page";
...
foreach($model as $row){ //Mostrar los datos encontrados en la BD $model, $row tomados lineas atras.
  ?>
      ...
      <td class="success"><?php echo $row['nie'];?></td>
    ...
    <?php
      ...
      if ($row['idestado'] =='1'){
        echo "<td align='center' width='10%' class='active'><button class='btn btn-danger btn-xs;' data-toggle='modal' data-target='#modal_rojo' onclick='desactivar(".$row['nie'].");''><i class='fa fa-trash-o'></i></button></td>";
      }
     ?>
      </tr>
      <!-- envio de datos-->
