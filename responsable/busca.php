...
/*Condicion para hacer la busqueda dentro de la paginación*/
  if($action == 'ajax'){
    ...
    //Arreglo de columnas donde se realizara la busqueda. Deacuerdo a las columnas
     $aColumns = array('dui','nombres', 'apellidos');//Columnas de busqueda
     $Table = "responsable"; //Nombre tabla de BD
...
//contamos los valores encontrados y realiza consulta.
    $count_query = "SELECT * FROM $Table $Where";
...
// Consulta de la paginación
      $sql = "SELECT *, dui FROM $Table $Where ORDER BY dui desc LIMIT $offset,$per_page";
...
<!-- Encabezada con efecto success-->
  ...
  <!-- style="text-align: center; Centrara el contenido de la columna principal ☺☺☺☺ -->
    <th style="text-align: center; width: 5px;">DUI</th> <!-- Encabezado de la fila principal de la tabla -->
...
foreach($model as $row){ //Mostrar los datos encontrados en la BD $model, $row tomados lineas atras.
  ?>
      ...
      <td class="success"><?php echo $row['dui'];?></td>
      <td><?php echo $row['nombres'];?></td>
      <td><?php echo $row['apellidos'];?></td>
      <td><?php echo $row['telefono'];?></td>
      <td><?php echo $row['direccion'];?></td>
      <?php
      echo "<td align='center' width='10%' class='active'><a class='btn btn-primary btn-xs;' data-toggle='modal' data-target='#modal_update' onclick='editar(".$row['dui'].");''><i style='color:white;' class='fa fa-edit'></a></i></td>";
     ?>
      </tr>
      <!-- envio de datos-->
      <input type="hidden" value="<?php echo $row['nombres'];?>" id="nombres<?php echo $row['dui'];?>">
      <input type="hidden" value="<?php echo $row['apellidos'];?>" id="apellidos<?php echo $row['dui'];?>">
      <input type="hidden" value="<?php echo $row['telefono'];?>" id="telefono<?php echo $row['dui'];?>">
      <input type="hidden" value="<?php echo $row['direccion'];?>" id="direccion<?php echo $row['dui'];?>">    <?php
