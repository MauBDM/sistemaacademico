...
//contamos los valores encontrados y realiza consulta.
      $count_query = "SELECT docente.*,estado.nombre as estado FROM $Table inner join estado on docente.idestado=estado.idestado $Where and docente.idestado='2'";
...
// Consulta de la paginación
      $sql = "SELECT docente.*,estado.nombre as estado FROM $Table inner join estado on docente.idestado=estado.idestado $Where and docente.idestado='2' ORDER BY docente.iddocente desc LIMIT $offset,$per_page";
...
foreach($model as $row){ //Mostrar los datos encontrados en la BD $model, $row tomados lineas atras.
  ?>
     <tr style="text-align: center;">
      <td class="danger"><?php echo $row['iddocente'];?></td>
    ...
    if ($row['estado'] =='Inactivo' ) {
      echo "<td align='center' width='10%' class='active'><button class='btn btn-success btn-xs;' data-toggle='modal' data-target='#modal_verde' onclick='activar(".$row['iddocente'].");''><i class='fa fa-check-circle'></i></button></td>";
    }
  ?>
      </tr>
      <!-- envio de datos-->
