...
//contamos los valores encontrados y realiza consulta.
    if ($idtipoUsu==1) {
      $count_query = "SELECT usuario.*,tipousuario.nombre as tiponombre FROM $Table inner join tipousuario on usuario.idtipoUsuario=tipousuario.idtipoUsuario $Where and usuario.estado='Inactivo'";
    }elseif ($idtipoUsu!=1) {
      $count_query = "SELECT usuario.*,tipousuario.nombre as tiponombre FROM $Table inner join tipousuario on usuario.idtipoUsuario=tipousuario.idtipoUsuario $Where and usuario.estado='Inactivo'";
    }
...
// Consulta de la paginación
    if ($idtipoUsu==1) {
      $sql = "SELECT usuario.*,tipousuario.nombre as tiponombre FROM $Table inner join tipousuario on usuario.idtipoUsuario=tipousuario.idtipoUsuario $Where and usuario.estado='Inactivo' ORDER BY usuario.idusuario desc LIMIT $offset,$per_page";
    }elseif ($idtipoUsu!=1) {
      $sql = "SELECT usuario.*,tipousuario.nombre as tiponombre FROM $Table inner join tipousuario on usuario.idtipoUsuario=tipousuario.idtipoUsuario $Where and usuario.estado='Inactivo' ORDER BY usuario.idusuario desc LIMIT $offset,$per_page";
    }
...
foreach($model as $row){ //Mostrar los datos encontrados en la BD $model, $row tomados lineas atras.
  ?>
      ...
      <td class="danger"><?php echo $row['idusuario'];?></td>
  ...
  <?php
    ...
    if ($row['estado'] =='Inactivo' ) {
      echo "<td align='center' width='10%' class='active'><button class='btn btn-success btn-xs;' data-toggle='modal' data-target='#modal_verde' onclick='activar(".$row['idusuario'].");''><i class='fa fa-check-circle'></i></button></td>";
    }
  ?>
      </tr>
      <!-- envio de datos-->
