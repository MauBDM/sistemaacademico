...
/*Condicion para hacer la busqueda dentro de la paginación*/
  if($action == 'ajax'){
    ...
    //Arreglo de columnas donde se realizara la busqueda. Deacuerdo a las columnas
     $aColumns = array('usuario.idusuario', 'usuario.usuario','tipousuario.nombre');//Columnas de busqueda
...
//contamos los valores encontrados y realiza consulta.
    if ($idusu==1) {
      $count_query = "SELECT usuario.*, tipousuario.nombre as tiponombre FROM $Table inner join tipousuario on usuario.idtipoUsuario=tipousuario.idtipoUsuario $Where and usuario.estado='Activo'";
    }elseif ($idusu!=1) {
      $count_query = "SELECT usuario.*,tipousuario.nombre as tiponombre FROM $Table inner join tipousuario on usuario.idtipoUsuario=tipousuario.idtipoUsuario $Where and usuario.idusuario='$idusu' and usuario.estado='Activo'";
    }
...
// Consulta de la paginación
    if ($idusu==1) {
      $sql = "SELECT usuario.*,DATE_FORMAT(ultimoIngreso, '%d/%m/%Y %H:%m:%s') AS miFecha, tipousuario.nombre as tiponombre FROM $Table inner join tipousuario on usuario.idtipoUsuario=tipousuario.idtipoUsuario $Where and usuario.estado='Activo' ORDER BY usuario.idusuario desc LIMIT $offset,$per_page";
    }elseif ($idusu!=1) {
      $sql = "SELECT usuario.*,DATE_FORMAT(ultimoIngreso, '%d/%m/%Y %H:%m:%s') AS miFecha,tipousuario.nombre as tiponombre FROM $Table inner join tipousuario on usuario.idtipoUsuario=tipousuario.idtipoUsuario $Where and usuario.idusuario='$idusu' and usuario.estado='Activo' ORDER BY usuario.idusuario desc LIMIT $offset,$per_page";
    }
